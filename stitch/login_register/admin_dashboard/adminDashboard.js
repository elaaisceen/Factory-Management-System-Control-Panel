...
function loadUserData() {
    ...
    if (userFullName && oturum.fullName) {
        userFullName.innerText = oturum.fullName;
    }
    if (userRole && oturum.roleName) {
        // Rol ismini daha okunabilir yap (GELISTIRICI_ADMIN -> Geliştirici / Admin)
        const roleDisplay = oturum.roleName.replace(/_/g, ' ').toLowerCase()
            .replace(/\b\w/g, l => l.toUpperCase());
        userRole.innerText = roleDisplay;
    }
}
            ...
            try {
                switch (currentDept) {
                    case 'production':
                        ...
                        logSystemEvent(JSON.parse(localStorage.getItem('oturum') || '{}').fullName || 'Sistem', `New Production: ${prodName}`, 'Production Sector', 'SUCCESS');
                        break;
                    case 'hr':
                        ...
                        logSystemEvent(JSON.parse(localStorage.getItem('oturum') || '{}').fullName || 'Sistem', `New Employee: ${pName}`, 'HR Office', 'SUCCESS');
                        break;
                    case 'finance':
                        const finDb = JSON.parse(localStorage.getItem('financeDatabase')) || [];
                        ...
                        logSystemEvent(JSON.parse(localStorage.getItem('oturum') || '{}').fullName || 'Sistem', `Finance Transaction: ${finComp}`, 'Finance Dept', 'SUCCESS');
                        break;
                    case 'stock':
                        ...
                        logSystemEvent(JSON.parse(localStorage.getItem('oturum') || '{}').fullName || 'Sistem', `Stock Update: ${stockName}`, 'Warehouse', 'SUCCESS');
                        break;
                    case 'it':
                        ...
                        logSystemEvent(JSON.parse(localStorage.getItem('oturum') || '{}').fullName || 'Sistem', `IT Ticket: ${itSubject}`, 'IT Desk', 'SUCCESS');
                        break;
                    case 'purchasing':
                        ...
                        logSystemEvent(JSON.parse(localStorage.getItem('oturum') || '{}').fullName || 'Sistem', `Purchasing: ${pureItem}`, 'Purchasing', 'SUCCESS');
                        break;
                }
