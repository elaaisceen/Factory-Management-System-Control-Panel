...
import com.factory.stitch.dao.DepartmentDashboardDao;
...
public class DepartmentService {
    ...
    private final DepartmentDashboardDao departmentDashboardDao;
    private final Map<String, DepartmentContext> departments = new LinkedHashMap<>();

    public DepartmentService(RoleMatrixService roleMatrixService, DepartmentDashboardDao departmentDashboardDao) {
        this.roleMatrixService = roleMatrixService;
        this.departmentDashboardDao = departmentDashboardDao;
    ...
    public List<DepartmentSummary> getVisibleDepartments(String role) {
        ...
        List<DepartmentSummary> dbDepartments = loadFromDatabase();
        if (!dbDepartments.isEmpty()) {
            return dbDepartments.stream()
                    .filter(summary -> visibleCodes.contains(normalizeCode(summary.getCode())))
                    .toList();
        }

        // Veritabani bos ise uygulamanin durmamasini saglayan fallback veri.
    ...
    public Optional<DepartmentSummary> getDepartmentStatus(String role, String departmentCode) {
        ...
        Optional<DepartmentSummary> dbSummary = loadFromDatabaseByCode(normalizedCode);
    ...
    private List<DepartmentSummary> loadFromDatabase() {
        try {
            return departmentDashboardDao.findAll();
        } catch (Exception ignored) {
            return Collections.emptyList();
        }
    }

    private Optional<DepartmentSummary> loadFromDatabaseByCode(String code) {
        try {
            return departmentDashboardDao.findByCode(code);
        } catch (Exception ignored) {
            return Optional.empty();
        }
    }
